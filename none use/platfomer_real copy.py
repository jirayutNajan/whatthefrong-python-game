...
text = End_font.render('The End', True, (0, 0, 0))
...
class boss():
    ...
    def draw(self):
        if player_rect.colliderect(self.place):
            ...
            test = self.time % 50
            if test == 0:
                boss_Bullets.append(boss_Bullet(self.rect.x + 48, self.rect.y + 48))
...
class boss_Bullet():
    def __init__(self, x, y):
        ...
        self.lifetime = 50
        self.speed = 15
        self.angle = math.atan2(player_rect.y + 16 - self.y,player_rect.x + 16 - self.x)
        self.x_vel = math.cos(self.angle) * self.speed
        self.y_vel = math.sin(self.angle) * self.speed
        self.radius = 10
